...
async def get_contact(message: types.Message, state: FSMContext):
    ...
    is_email = re.fullmatch(
        r"[a-zA-Z0-9._%+-]+@(gmail\.com|mail\.ru|yandex\.ru)", contact, re.IGNORECASE
    )
    is_phone_plus = re.fullmatch(r"\+79\d{9}", contact)   # +79XXXXXXXXX
    is_phone_8 = re.fullmatch(r"89\d{9}", contact)        # 89XXXXXXXXX

    # ----- Валидация -----
    if is_phone_8:
        contact = "+7" + contact[1:]  # Преобразуем в +79XXXXXXXXX
    elif not (is_email or is_phone_plus):
        await message.answer(
            "❌ Введи корректный email (tata@gmail.com, tata@mail.ru, tata@yandex.ru) "
            "или номер телефона в формате +79XXXXXXXXX или 89XXXXXXXXX (вторая цифра — 9)."
        )
        return
...
from app.core.session import async_session_maker
from app.repositories.user_repository import UserRepository
from app.models.validation import ContactValidator
...
async def get_contact(message: types.Message, state: FSMContext):
    try:
        validated = ContactValidator(contact=message.text)
